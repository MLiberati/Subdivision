## Part 3B - Genetic Algorithm for Post-subdivision parcelization in Protect Area Expansion Demo Area
print "PART 3B - Genetic Algorithm for Post-subdivision landscapes for PA expansion"

## Adapted "inspyred_NSGAII_adaptedthread.py"
## MRL adapted Stabilization_NSGAII_ALLiteration_2017.10.23.py script
## Apply algorithm to each subdivided town in MA
...
# Results data
resultsFolder = "Sub_Results\POST_COMBO_Originals"
resultsPath = r"{0}\{1}".format(homeFolder,resultsFolder)
if not os.path.isdir(resultsPath):
    os.makedirs(resultsPath)

# Additional results folders
resultsPath = r"{0}\{1}".format(homeFolder,"Sub_Results\POST_COMBO_forR")
if not os.path.isdir(resultsPath):
    os.makedirs(resultsPath)
...
# Subidivided lots with updated attributes in demo area
finalFC = r"{0}\{1}\{2}\demoArea_POST_combo_7500m2.shp".format(homeFolder,derivedFolder,demoFolder)
memory_finalFC = r"in_memory\demoArea_POST_combo"
arcpy.CopyFeatures_management(finalFC, memory_finalFC)
...
fields = ("TOWN_ID","FID","LOTSIZE_HA","SUBLOTCOST","WghtByArea","RIPAR_HA","COMPACT","PRIORTY_HA")
# NOTE - SUBLOTCOST replaces LOT_COST from pre-subdiv script

rowsAVAIL = arcpy.da.SearchCursor(memory_finalFC, fields)
for row in rowsAVAIL:
    ...
    # add parcel information to correct town (dictionary key)
...
print "\nThere are {0} parcels available for selection in the POST-subdivided landscape".format(parcCnt)

print "\nEXPLANATION OF THE NESTED LIST STRUCTURE"
print "Values are a nested list - number of lists = number of parcels within the town"
print "List for each parcel inclues the following attributes (in order):"
print "0) Town ID,"
print "1) FID,"
print "2) Lot size,"
print "3) Cost,"
...
POSThabLst = [V6, V10, V15, V16, V21, V22, V23]
POSThabParcels = []

POSThab = r"{0}\{1}\{2}\zonal_HABinPOST.dbf".format(homeFolder,derivedFolder,demoFolder)
field_zonal = [f.name for f in arcpy.ListFields(POSThab)]

rowsAVAIL = arcpy.da.SearchCursor(POSThab, field_zonal)
for row in rowsAVAIL:
    ...
    POSThabParcels.append([FID,v1,v3,v4,v6,v10,v15,v16,v21,v22,v23])

    for i,value in enumerate(values):

        if value in field_zonal:
            POSThabLst[i] += row[field_zonal.index(value)]
        if value not in field_zonal:
            POSThabLst[i] += float(0)
...
print "\nCreated nested list for habitat info for each POST parcel"
...
target_percentRep = []
for i,amount in enumerate(POSThabLst):
    percent = (amount + PAhabLst[i]) / (POST_area + PA_area) * 100
    ...
    for c in candidates:
        ...
        for parcelID,parcel in enumerate(c):
            ...
            if parcel == 1:
                ...
                tot_exclude += POSThabParcels[parcelID][1] + POSThabParcels[parcelID][2] + POSThabParcels[parcelID][3]
                tot_V6 += POSThabParcels[parcelID][4]
                tot_V10 += POSThabParcels[parcelID][5]
                tot_V15 += POSThabParcels[parcelID][6]
                tot_V16 += POSThabParcels[parcelID][7]
                tot_V21 += POSThabParcels[parcelID][8]
                tot_V22 += POSThabParcels[parcelID][9]
                tot_V23 += POSThabParcels[parcelID][10]
        ...
        if(len(connectLst)>0):
            ...
            arcpy.MakeFeatureLayer_management(memory_finalFC, memory_subAVAIL, where_clause=query)        
...
def CustomObserver(population, num_generations, num_evaluations, args):
    ...
    filename = r"{0}\{1}\{2}gens_POST_{3}.csv".format(homeFolder, resultsFolder, num_generations, date)
